...
Select email,
Left(email,1)||'***.'|| Substring (email From Position('.' In email)+1 For 1)||'***'|| Substring(email From position('@' In email)) As new_email
From customer
...
Select email, 
'***'|| Substring(email From Position('.' In email)-1 For 1) ||'.***' ||Substring(email From Position('@' In email)-1 For 1)|| 
